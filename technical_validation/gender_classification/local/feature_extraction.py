...
def compute_w2v(file_path, sampling_rate):
    try:
        # Load audio file
        ...
        audio = None
        print("Read audio failed for " + file_path)
    return audio, file_path.split('/')[-2]
...
def main(config, in_wav_list, out_folder):
    ...
    feats_list = []
    bad_ids_list = []
    fff = config['default']['feature_type']
    print(fff)
    for file_id, file_path in in_wav_list:
        if config['default']['feature_type'] == 'w2v':
            s, aud_id = compute_w2v(file_path, int(config['default']['sampling_rate']))
            if s is None:
